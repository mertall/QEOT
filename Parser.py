...
def find_depth(cloud_dict):
## create depednancy tree and then use depth firstsearch counting how deep the treee is 

def understandability(cloud_dict): ##lol, go through each documents and finds ratio of words that are not understandable
    iter = len(cloud_dict)
    counter = 0
    score =[]
    for i in range(iter):
